...
// -------------------- FUNCIONES DFCW SOFT --------------------
void sendSymbol_SoftDFCW(char s) {
    int64_t startFreq, endFreq;
    int duration;
    ...
    int steps = 50;
    int64_t freqStep = (endFreq - startFreq) / steps;
    ...
    for (int i = 0; i < steps; i++) {
        int64_t f = startFreq + i * freqStep;
        si5351.set_freq(f, SI5351_CLK1);
        delay(stepDelay);
    }

    // Mantener frecuencia final 1 paso
    si5351.set_freq(endFreq, SI5351_CLK1);
    delay(stepDelay);
...
void sendChar_SoftDFCW(const char* morse) {
    for (int i = 0; morse[i] != 0; i++) {
        sendSymbol_SoftDFCW(morse[i]);
    }
    delay(char_pause);
}
...
void setup() {
  ...
  display.println("DFCW SOFT EA5JTT");
  display.display();
  // SI5351
